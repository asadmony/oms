        ...
        <div class="row mb-2">
          ...
          <div class="col-md-5">
              <div class="input-group mb-2">
                <input type="text" class="form-control" name="query" id="searchQuery" placeholder="Name, Mobile, Role name, NID">
       ...
       @include('alerts.alerts')
      	...
      	<div class="card card-widget">
            <div class="card-header with-border">
              <h3 class="card-title"><i class="fa fa-th"></i> All @if ($type == 'depo')
                  GM
            @elseif($type == 'distributor')
            Distributor
            @elseif($type == 'dealer')
          ...
          <tbody id="tableBody">
			      @include('admin.roles.parts.rolesTableBody')
          </tbody>
			  </table>
            </div>
...
<div id="myModal" class="modal fade" role="dialog">
  ...
  <div class="form-group has-feedback{{ $errors->has('user') ? ' has-error' : '' }}" id="userField">
...
<div class="form-group has-feedback{{ $errors->has('depo') ? ' has-error' : '' }}">
    <label for="depo">{{ __('GM') }}:</label>

     <select class="form-control select2-" name="depo" id="depo">
      <option value="">{{ __('Select GM') }}</option>
...
<div class=" form-group has-feedback{{ $errors->has('monthly_target') ? ' has-error' : '' }}">
    <label for="monthly_target">Monthly Target:</label>
   <input class="form-control" type="number" name="monthly_target" id="monthly_target" value="{{ old('monthly_target') }}" placeholder="ex. 100000">
</div>
...
<div class="form-group has-feedback{{ $errors->has('nid ') ? ' has-error' : '' }}">
  ...
  <input type="text" class="form-control" name="nid" id="nid" value="{{ old('nid') }}" placeholder="ex. 13XXXXXXXXXX" required>
</div>
...
<div class="form-group has-feedback{{ $errors->has('title ') ? ' has-error' : '' }}">
    <label for="title">
      ...
      <input type="text" name="title" value="{{ old('title') }}" class="form-control" placeholder="@if ($type == 'depo') GM @elseif($type == 'distributor') Distributor @elseif($type == 'dealer') SMO @elseif($type == 'agent') SR @else @endif Name">
...
<div class="form-group pb-3 border-bottom">
  <div class="input-group">
      ...
      <label class="col-sm-5 form-check-label" for="active">
        <input class="form-check-input" type="checkbox" id="active_cat_{{ $category->id }}" checked onchange="toggleCatComission({{ $category->id }},{{ $category->sr_commission }})">
          {{ $category->name }}
      </label>
      <input class="col-sm-7 form-control" type="number" min="0"  max="100" step="0.01" value="{{ $category->sr_commission }}" name="category_{{ $category->id }}" id="category_{{ $category->id }}">
      <div class="input-group-append">
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/axios/0.21.1/axios.min.js" integrity="sha512-bZS47S7sPOxkjU/4Bt0zrhEtWx0y0CRkhEp8IckzK+ltifIIE9EMIMTuT/mEzoIMewUINruDBIR/jJnbguonqQ==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
        <script>
          ...
          $('#distributor').on('change',function(){
            ...
            $.ajax({url: `/ajax/distributor/${distributor}/upazila/get`, success: function(result){
          ...
          $('#depo').on('change',function(){
            var depo = $('#depo').val();
            $.ajax({url: `/ajax/depo/${depo}/district/get`, success: function(result){
              $('#district').html(`
              `)
              result.forEach(element => {
                $('#district').append(`
