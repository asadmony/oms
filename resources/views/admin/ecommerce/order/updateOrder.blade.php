                                ...
                                <table class="table invoice-table table-sm table-striped table-bordered">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                {{ $order->orderByAgent->user->name }} ({{$order->orderByAgent->name}})
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                {{ $order->orderByAgent->dealer->user->name }} ({{ $order->orderByAgent->dealer->name }})
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header w3-blue">
                              <h3 class="card-title ">
                                      Manage Product | Order Status : <span class="badge @if($order->order_status == 'confirmed') badge-success @else badge-primary @endif ">{{ Str::ucfirst($order->order_status) }}</span>
                              </h3>
                            </div>
                            <!-- /.card-header -->
                            
                            <form class="form-inline" method="post" action="{{ route('admin.ecommerce.orderOrderStatusUpdate',$order) }}">
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                              ...
                                              <tr>
                                                ...
                                                <td>
                                                    <img style="max-width: 200px;max-height: 100px; margin:auto;" src="{{ $item->product->feature_img }}" alt="">
                                                </td>
                                                ...
                                                <td>
                                                    <input type="number" class="form-control" name="order_qty_{{ $item->id }}" value="{{ $item->total_quantity }}" id="order_qty_{{ $item->id }}">
                                                    <button class="btn btn-danger" onclick="event.preventDefault();removeProduct({{ $item->id }})"><i class="fa fa-trash"></i></button>
                                                </td>
                                              </tr>
                                              @endforeach
                                              <tr>
                                                  ...
                                                  <td>
                                                    <div class="input-group m-auto input-group-sm">
                                                        ...
                                                        {{$order->order_status == 'delivered' ? 'disabled' : ''}} {{$order->order_status == 'confirmed' ? 'disabled' : ''}} {{$order->order_status == 'confirmed' ? 'processing' : ''}} {{$order->order_status == 'confirmed' ? 'ready_to_ship' : ''}} {{$order->order_status == 'confirmed' ? 'shipped' : ''}} {{$order->order_status == 'collected' ? 'disabled' : ''}}>
                                                          ...
                                                          <option {{ $order->order_status == 'confirmed' ? 'selected' : '' }} value="confirmed">Confirm</option>
                                                          ...
                                                          <option {{ $order->order_status == 'cancelled' ? 'selected' : '' }} value="cancelled">Cancel</option>
              
              
                                                      </select>
                                                      <div class="input-group-append">
                                                          <button class="btn btn-success" type="submit" {{$order->order_status == 'delivered' ? 'disabled' : ''}} {{$order->order_status == 'confirmed' ? 'disabled' : ''}} {{$order->order_status == 'confirmed' ? 'processing' : ''}} {{$order->order_status == 'confirmed' ? 'ready_to_ship' : ''}} {{$order->order_status == 'confirmed' ? 'shipped' : ''}} {{$order->order_status == 'collected' ? 'disabled' : ''}}>Update</button>
                                                      </div>
                                                  </div>
                                                  </td>
                                              </tr>
